...
1. 顺序存储结构
    ...
    对每个线性表位置的存入或者取出数据，对于计算机来说都是相等的时间，也就是一个常数，存取时间性能为 **O(1)** 。具有这一特点的存储结构称为**随机存取结构**。
...
2. 链式存储结构
    ...
    把存储数据元素信息的域称为**数据域**，把存储直接后继位置的域称为**指针域**。指针域中存储的信息称做指针或链。这两部分信息组成数据元素的存储映像，称为**结点（Node）**。
